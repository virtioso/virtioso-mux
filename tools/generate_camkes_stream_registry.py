...
def _demux_sinks(preprocessed, stream_by_component):
    ...
    for match in CONNECTION_RE.finditer(composition):
        from_component, from_interface, to_component, to_interface = match.groups()
        if "getchar" not in to_interface:
            continue
        ...
        client_index = sinks_by_endpoint.get(target_key, 0)
        sinks_by_endpoint[target_key] = client_index + 1
        sinks.append(
            {
                "demux_component": to_component,
                "demux_interface": to_interface,
                "demux_client_index": client_index,
                "component": from_component,
                "stream_id": stream["stream_id"],
            }
        )
...
def _write_header(path, registry):
    ...
    lines.extend(
        [
            ...
            "};",
            "",
            "typedef void *(*virtioso_camkes_demux_buf_fn_t)(void);",
            "typedef void (*virtioso_camkes_demux_emit_fn_t)(void);",
            "",
            ...
            "    virtioso_camkes_demux_buf_fn_t buf;",
            "    virtioso_camkes_demux_emit_fn_t emit;",
            "};",
            "",
...
def _write_c(path, header_name, registry):
    ...
    for sink in registry["demux_sinks"]:
        interface = _sanitize_c_ident(sink["demux_interface"])
        wrapper = _sanitize_c_ident(
            "virtioso_demux_%s_%s_%s"
            % (sink["demux_component"], sink["demux_interface"], sink["component"])
        )
        index = int(sink["demux_client_index"])
        lines.extend(
            [
                "extern unsigned int %s_num_badges(void);" % interface,
                "extern seL4_Word %s_enumerate_badge(unsigned int i);" % interface,
                "extern void *%s_buf(seL4_Word client_id);" % interface,
                "extern void %s_emit(unsigned int badge);" % interface,
                "",
                "static seL4_Word %s_badge(void)" % wrapper,
                "{",
                "    if (%s_num_badges() <= %d) {" % (interface, index),
                "        return (seL4_Word)-1;",
                "    }",
                "    return %s_enumerate_badge(%d);" % (interface, index),
                "}",
                "",
                "static void *%s_buf(void)" % wrapper,
                "{",
                "    seL4_Word badge = %s_badge();" % wrapper,
                "    if (badge == (seL4_Word)-1) {",
                "        return 0;",
                "    }",
                "    return %s_buf(badge);" % interface,
                "}",
                "",
                "static void %s_emit(void)" % wrapper,
                "{",
                "    seL4_Word badge = %s_badge();" % wrapper,
                "    if (badge == (seL4_Word)-1) {",
                "        return;",
                "    }",
                "    %s_emit((unsigned int)badge);" % interface,
                "}",
                "",
            ]
        )
